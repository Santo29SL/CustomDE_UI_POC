...
def run_etl():
    ...
    print("📤 Loading records into postgres: es_bronze.goals...")
    ...
    try:
        with pg_engine.connect() as conn:
            result = conn.execute(text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'es_bronze' AND table_name = 'goals');"))
    ...
    with pg_engine.begin() as conn:
        ...
        if table_exists:
            conn.execute(text("TRUNCATE TABLE es_bronze.goals CASCADE;"))
        df.to_sql(name="goals", con=conn, schema="es_bronze", if_exists="append", index=False)
