...
MONGO_URI = "mongodb://localhost:27017/expendsave"
...
def run_etl():
    ...
    collection = mongo_client[db_name]["investmentschemes"]
    
    print("📥 Reading data from collection: investmentschemes...")
    ...
    print("📤 Loading records into postgres: expendsave_bronze.investmentschemes...")
    ...
    try:
        with pg_engine.connect() as conn:
            result = conn.execute(text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'expendsave_bronze' AND table_name = 'investmentschemes');"))
    ...
    with pg_engine.begin() as conn:
        ...
        if table_exists:
            conn.execute(text("TRUNCATE TABLE expendsave_bronze.investmentschemes CASCADE;"))
        df.to_sql(name="investmentschemes", con=conn, schema="expendsave_bronze", if_exists="append", index=False)
