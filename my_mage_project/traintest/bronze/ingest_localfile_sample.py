...
FILE_PATH = "/Users/santhoshsl/Custom_GUI_Airflow/my_mage_project/traintest/bronze/data/sample.csv"

# Docker DNS and filepath overrides
if os.getenv("DOCKER_ENV") == "true":
    POSTGRES_URI = POSTGRES_URI.replace("localhost", "host.docker.internal")
    FILE_PATH = "/home/src/my_mage_project/traintest/bronze/data/sample.csv"
...
def run_etl():
    ...
    print("📤 Loading records into postgres: traintest_bronze.sample...")
    ...
    try:
        with pg_engine.connect() as conn:
            result = conn.execute(text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'traintest_bronze' AND table_name = 'sample');"))
    ...
    with pg_engine.begin() as conn:
        ...
        if table_exists:
            conn.execute(text("TRUNCATE TABLE traintest_bronze.sample CASCADE;"))
        df.to_sql(name="sample", con=conn, schema="traintest_bronze", if_exists="append", index=False)
