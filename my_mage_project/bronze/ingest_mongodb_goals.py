...
def run_etl():
    ...
    collection = mongo_client[db_name]["goals"]
    
    print("📥 Reading data from collection: goals...")
    ...
    print("📤 Loading records into postgres: expendsave_bronze.goals...")
    ...
    try:
        with pg_engine.connect() as conn:
            result = conn.execute(text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'expendsave_bronze' AND table_name = 'goals');"))
    ...
    with pg_engine.begin() as conn:
        ...
        if table_exists:
            conn.execute(text("TRUNCATE TABLE expendsave_bronze.goals CASCADE;"))
        df.to_sql(name="goals", con=conn, schema="expendsave_bronze", if_exists="append", index=False)
